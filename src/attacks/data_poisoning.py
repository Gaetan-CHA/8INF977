...
Category: Dataset-level\ backdoor\ /\ poisoning
Source: Tay\ Bot\ incident\ \(2016\)\ \+\ NeurIPS\ 2024\ \ 'Mission\ Impossible'\ paper

Short description:
Simulation\ d'injection\ de\ données\ malveillantes\ dans\ le\ corpus\ d'entraînement\.
...
def generate(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "prompt": prompt,
        "metadata": {
            "attack_id": "data_poisoning\.py",
            "title": "Data\ Poisoning\ \(training-time\)",
            "source": "Tay\ Bot\ incident\ \(2016\)\ \+\ NeurIPS\ 2024\ \ 'Mission\ Impossible'\ paper"
        }
    }
