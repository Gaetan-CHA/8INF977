...
Usage
  python -m src.protocol.test_models --yaml .\configs\models_api.yaml --out out
...
try:
    from huggingface_hub import HfApi, snapshot_download  # ok for >=0.36
    try:
...
def print_install_procedure() -> None:
...
def print_no_token_help() -> None:
    ...
    print('   python -c "import keyring; t=keyring.get_password(\'hf\',\'HF_TOKEN\'); keyring.set_password(\'huggingface\',\'token\',t) if t else print(\'no token\')"')
    print("D) Vérifier :")
    print('   python -c "import os,keyring; print(\'env:\', bool(os.getenv(\'HUGGINGFACEHUB_API_TOKEN\') or os.getenv(\'HF_TOKEN\'))); ' \
...
def read_models_yaml(path: Path) -> List[ModelSpec]:
    ...
    1) {"models": [{"id": "org/repo", ...}, ...]}
    2) {"model_1": {"model_name": "org/repo", ...}, ...}
    """
    ...
    for m in data.get("models", []):
        models.append(ModelSpec(
            ...
            model_id=m.get("id") or m.get("model_id") or m.get("model_name") or "",
    ...
    for k, v in data.items():
        if isinstance(v, dict) and str(k).startswith("model_") and ("model_name" in v or "id" in v):
            models.append(ModelSpec(
                enterprise=v.get("enterprise") or v.get("publisher") or "",
                model_id=v.get("model_name") or v.get("id") or "",
                params_b=v.get("params_b"),
...
def get_repo_type(api: HfApi, model_id: str, token: Optional[str]) -> Tuple[str, str]:
    """Returns (repo_type, note) where repo_type in {gated, public, unknown}."""
    try:
...
def check_access(api: HfApi, model_id: str, token: Optional[str]) -> Tuple[str, str]:
    """Returns (access, reason) where access in {ok, no, unknown}."""
    try:
...
def save_outputs(rows: List[dict], out_dir: Path) -> None:
    ...
    if tabulate:
        table = tabulate([list(r.values()) for r in rows], headers=list(rows[0].keys()), tablefmt="github")
    else:
...
def resolve_yaml_path(cli_value: Optional[str]) -> Path:
    if cli_value:
    ...
    candidates = [
        Path.cwd() / "configs" / "models_api.yaml",          # exécution depuis la racine
        here.parents[2] / "configs" / "models_api.yaml",     # <repo>/configs (src/protocol/test_models.py)
        here.parents[1] / "configs" / "models_api.yaml",     # src/configs
        here.parent / "configs" / "models_api.yaml",         # src/protocol/configs
    ]
    for p in candidates:
        if p.exists():
            return p
    raise FileNotFoundError("models_api.yaml introuvable. Essayez --yaml .\\configs\\models_api.yaml")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--yaml", default=None, help="Path to models_api.yaml")
    ...
    # 3) Load YAML with robust resolution
    try:
    ...
    print(f"YAML path resolved to: {yaml_path}")
    ...
    rows: List[dict] = []
    _, token_val = have_api()
    for m in specs:
        chk = check_model(m, token_val)
        row = {
        ...
        rows.append(row)
        print(f" - {m.model_id:45s} installed={row['installed']:<3s} remote={row['remote']} repo_type={row['repo_type']} access={row['access']} {('('+row['note']+')') if row['note'] else ''}")
