...
# - load_json() : Charge un fichier JSON en dictionnaire.
...
CFG_MODELS = Path(__file__).resolve().parents[2] / "configs" / "models_azure.json"
CFG_ATTACKS = Path(__file__).resolve().parents[2] / "configs" / "attacks.json"
CFG_DEFENSES = Path(__file__).resolve().parents[2] / "configs" / "defenses.json"
CFG_RUNNER  = Path(__file__).resolve().parents[2] / "configs" / "runner_attacks.json"
...
# ===========================load_json===========================
## Pourquoi :
# Charger un fichier JSON en dict de façon tolérante, avec traces optionnelles.
## Input :
# p : Path — chemin du fichier JSON.
## Output :
...
def load_json(p: Path) -> Any:
    if not p.exists():
        if DEBUG_JSON:
            print(f"[DEBUG_JSON] fichier absent: {p}")
        return {}
    ...
    try:
        ...
        raise RuntimeError(f"JSON invalide: {p} — {e}") from e
    if DEBUG_JSON:
        print(f"[DEBUG_JSON] Chargé: {p.resolve()}")
        print(f"[DEBUG_JSON] Preview: {str(data)[:1000]}")
    return data
...
class AzureClient:
    def __init__(self, *, endpoint: str, api_key: str, api_version: str,
                 ...
                 max_completion_tokens: int = 256, top_p: float = 1.0,
...
# ===========================extract_models===========================
## Pourquoi : ##
# Normaliser et filtrer la liste des modèles depuis le JSON.
## Quand : ##
# Juste après lecture de configs modèles.
## Input : ##
# cfg : dict — structure issue de models_*.JSON.
##Output##
...
def extract_models(cfg: dict) -> List[dict]:
    ...
    for m in (cfg or {}).get("models", []):
        ...
        out.append({
            ...
            "max_completion_tokens": int(m.get("max_completion_tokens", 256)),
    ...
    if DEBUG_JSON:
        print(f"[DEBUG_JSON] extract_models -> {len(out)} modèles trouvés")
    return out
...
## Input : ##
# Aucun input direct; lit les fichiers JSON référencés en haut de fichier.
##Output##
...
def main():
    ...
    cfg_models = load_json(CFG_MODELS)
    cfg_runner = load_json(CFG_RUNNER)
    cfg_attacks = load_json(CFG_ATTACKS)
    cfg_defenses = load_json(CFG_DEFENSES)

    if DEBUG_JSON:
        print(f"[DEBUG_JSON] runner config preview: {str(cfg_runner)[:1000]}")
    ...
    # Préparer le catalogue des attaques depuis le JSON.
    ...
    # Base prompt optionnel depuis un fichier de prompts (JSON uniquement).
    base_prompt = None
    pf = cfg_runner.get("prompts_file")
    if pf:
        p = Path(pf)
        if p.suffix.lower() != ".json":
            raise SystemExit(f"prompts_file doit être un .json, reçu: {p}")
        if not p.exists():
            raise SystemExit(f"prompts_file introuvable: {p}")
        data = load_json(p)
